...
# install spark 1.6.1 pre-built for hadoop 2.6
if [ ! -f "/vagrant/resources/spark-1.6.1-bin-hadoop2.6.tgz" ]
then
	wget --progress=bar:force http://ftp.wayne.edu/apache/spark/spark-1.6.1/spark-1.6.1-bin-hadoop2.6.tgz
	sudo cp spark-1.6.1-bin-hadoop2.6.tgz /vagrant/resources
fi

sudo tar -zxf /vagrant/resources/spark-1.6.1-bin-hadoop2.6.tgz
...
if [ ! -f "/vagrant/resources/zeppelin-0.5.6-incubating-bin-all.tgz" ]
then    
    wget --progress=bar:force http://www.gtlib.gatech.edu/pub/apache/incubator/zeppelin/0.5.6-incubating/zeppelin-0.5.6-incubating-bin-all.tgz
    sudo cp zeppelin-0.5.6-incubating-bin-all.tgz /vagrant/resources
fi

sudo tar -zxf /vagrant/resources/zeppelin-0.5.6-incubating-bin-all.tgz
